import { useState, useRef, useEffect } from "react";
import { MoreVertical, Eye, Edit2, Play, Ban, CheckCircle, XCircle, Copy, Trash2, Calendar, ShieldAlert } from "lucide-react";
import type { Event } from "../types/event";

interface EventTableProps {
  events: Event[];
  onView: (event: Event) => void;
  onEdit: (event: Event) => void;
  onDelete: (event: Event) => void;
  onCancel: (event: Event) => void;
  onPublish: (event: Event) => void;
  onApprove: (event: Event) => void;
  onReject: (event: Event) => void;
  onDuplicate: (event: Event) => void;
  isPending?: boolean;
}

export function EventTable({
  events,
  onView,
  onEdit,
  onDelete,
  onCancel,
  onPublish,
  onApprove,
  onReject,
  onDuplicate,
  isPending = false,
}: EventTableProps) {
  const [activeMenuId, setActiveMenuId] = useState<string | null>(null);
  const menuRef = useRef<HTMLDivElement>(null);

  // Close dropdown on outside click
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (menuRef.current && !menuRef.current.contains(event.target as Node)) {
        setActiveMenuId(null);
      }
    }
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 border border-slate-200 dark:border-white/5 rounded-2xl bg-white dark:bg-ink-900">
        <ShieldAlert size={36} className="text-slate-300 dark:text-slate-700 mb-3 animate-pulse" />
        <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">No events found</h4>
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
          Create a new event or change search filters.
        </p>
      </div>
    );
  }

  const getStatusStyles = (status: string) => {
    switch (status) {
      case "UPCOMING":
        return "bg-indigo-50 text-indigo-700 border border-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-none";
      case "ONGOING":
        return "bg-emerald-50 text-emerald-700 border border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-none";
      case "COMPLETED":
        return "bg-slate-100 text-slate-700 border border-slate-200 dark:bg-white/[0.04] dark:text-slate-400 dark:border-none";
      case "CANCELLED":
        return "bg-rose-50 text-rose-700 border border-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:border-none";
      default:
        return "bg-slate-50 text-slate-650";
    }
  };

  const getApprovalStyles = (status: string) => {
    switch (status) {
      case "APPROVED":
        return "bg-emerald-50 text-emerald-700 border border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-none";
      case "PENDING":
        return "bg-amber-50 text-amber-700 border border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-none";
      case "REJECTED":
        return "bg-rose-50 text-rose-700 border border-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:border-none";
      default:
        return "bg-slate-50 text-slate-650";
    }
  };

  return (
    <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/5 dark:bg-ink-900 transition-colors duration-300">
      <table className="w-full text-left text-xs border-collapse">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50/70 font-semibold tracking-wider text-slate-400 dark:border-white/5 dark:bg-white/[0.01] dark:text-slate-500 uppercase">
            <th className="px-6 py-4">Event details</th>
            <th className="px-6 py-4">Category</th>
            <th className="px-6 py-4">Organizer & Dept</th>
            <th className="px-6 py-4">Schedule & Venue</th>
            <th className="px-6 py-4">Registrations</th>
            <th className="px-6 py-4">Status & Approval</th>
            <th className="px-6 py-4 text-right">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
          {events.map((event) => {
            const initials = event.title
              ? event.title
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
                  .toUpperCase()
                  .slice(0, 2)
              : "EV";

            return (
              <tr
                key={event._id}
                className="hover:bg-slate-50/30 dark:hover:bg-white/[0.01] transition-colors"
              >
                {/* Event Name & Banner */}
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    {event.bannerImage ? (
                      <img
                        src={event.bannerImage}
                        alt=""
                        className="size-10 rounded-xl object-cover border border-slate-100 dark:border-white/5 shrink-0"
                      />
                    ) : (
                      <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-indigo-500/10 to-violet-500/10 text-[11px] font-bold text-indigo-650 dark:text-indigo-400">
                        {initials}
                      </div>
                    )}
                    <div className="min-w-0">
                      <span className="block font-semibold text-slate-900 dark:text-white truncate">
                        {event.title}
                      </span>
                      <span className="block text-[10px] text-slate-450 dark:text-slate-500 truncate mt-0.5">
                        Time: {event.time}
                      </span>
                    </div>
                  </div>
                </td>

                {/* Category */}
                <td className="px-6 py-4">
                  <span className="rounded bg-indigo-50 px-2 py-0.5 font-bold uppercase text-[9px] tracking-wider text-indigo-650 dark:bg-indigo-500/10 dark:text-indigo-400">
                    {event.category}
                  </span>
                </td>

                {/* Organizer & Dept */}
                <td className="px-6 py-4">
                  <span className="block font-semibold text-slate-750 dark:text-slate-300 truncate max-w-[130px]">
                    {event.organizer}
                  </span>
                  <span className="block text-[10px] text-slate-400 dark:text-slate-500 truncate max-w-[130px] mt-0.5">
                    {event.department}
                  </span>
                </td>

                {/* Schedule & Venue */}
                <td className="px-6 py-4">
                  <span className="block font-semibold text-slate-750 dark:text-slate-300">
                    {new Date(event.date).toLocaleDateString()}
                  </span>
                  <span className="block text-[10px] text-slate-400 dark:text-slate-500 truncate max-w-[120px] mt-0.5">
                    {event.venue}
                  </span>
                </td>

                {/* Registration Count */}
                <td className="px-6 py-4">
                  <div className="min-w-0">
                    <span className="block font-bold text-slate-750 dark:text-slate-300">
                      {event.currentRegistrations}
                    </span>
                    <span className="block text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">
                      Max: {event.maxParticipants}
                    </span>
                  </div>
                </td>

                {/* Status & Approval */}
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1 items-start">
                    <span
                      className={`rounded-lg px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${getStatusStyles(
                        event.status
                      )}`}
                    >
                      {event.status}
                    </span>
                    <span
                      className={`rounded-lg px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${getApprovalStyles(
                        event.approvalStatus
                      )}`}
                    >
                      {event.approvalStatus}
                    </span>
                  </div>
                </td>

                {/* Actions Dropdown */}
                <td className="px-6 py-4 text-right relative">
                  <button
                    onClick={() =>
                      setActiveMenuId(activeMenuId === event._id ? null : event._id)
                    }
                    disabled={isPending}
                    className="inline-flex size-7 items-center justify-center rounded-lg border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] text-slate-400 hover:text-slate-800 dark:hover:text-white transition-all hover:shadow-sm"
                  >
                    <MoreVertical size={14} />
                  </button>

                  {/* Context menu */}
                  {activeMenuId === event._id && (
                    <div
                      ref={menuRef}
                      className="absolute right-6 top-12 z-20 w-44 origin-top-right rounded-2xl border border-slate-200/80 bg-white p-2 shadow-xl dark:border-white/5 dark:bg-ink-900 animate-fade-up text-left"
                    >
                      <button
                        onClick={() => {
                          setActiveMenuId(null);
                          onView(event);
                        }}
                        className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-slate-650 hover:bg-slate-50 dark:text-slate-355 dark:hover:bg-white/[0.03] transition-all"
                      >
                        <Eye size={13} />
                        View Event
                      </button>

                      <button
                        onClick={() => {
                          setActiveMenuId(null);
                          onEdit(event);
                        }}
                        className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-slate-650 hover:bg-slate-50 dark:text-slate-355 dark:hover:bg-white/[0.03] transition-all"
                      >
                        <Edit2 size={13} />
                        Edit Event
                      </button>

                      {/* Publish / Cancel toggle */}
                      {event.status === "CANCELLED" ? (
                        <button
                          onClick={() => {
                            setActiveMenuId(null);
                            onPublish(event);
                          }}
                          className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-emerald-600 hover:bg-emerald-50 dark:text-emerald-450 dark:hover:bg-emerald-500/10 transition-all"
                        >
                          <Play size={13} />
                          Publish Event
                        </button>
                      ) : (
                        event.status !== "COMPLETED" && (
                          <button
                            onClick={() => {
                              setActiveMenuId(null);
                              onCancel(event);
                            }}
                            className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-amber-600 hover:bg-amber-50 dark:text-amber-450 dark:hover:bg-amber-500/10 transition-all"
                          >
                            <Ban size={13} />
                            Cancel Event
                          </button>
                        )
                      )}

                      {/* Approval flow */}
                      {event.approvalStatus === "PENDING" && (
                        <>
                          <button
                            onClick={() => {
                              setActiveMenuId(null);
                              onApprove(event);
                            }}
                            className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-emerald-600 hover:bg-emerald-50 dark:text-emerald-450 dark:hover:bg-emerald-500/10 transition-all"
                          >
                            <CheckCircle size={13} />
                            Approve Event
                          </button>
                          <button
                            onClick={() => {
                              setActiveMenuId(null);
                              onReject(event);
                            }}
                            className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all"
                          >
                            <XCircle size={13} />
                            Reject Event
                          </button>
                        </>
                      )}

                      <button
                        onClick={() => {
                          setActiveMenuId(null);
                          onDuplicate(event);
                        }}
                        className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-slate-650 hover:bg-slate-50 dark:text-slate-355 dark:hover:bg-white/[0.03] transition-all"
                      >
                        <Copy size={13} />
                        Duplicate Event
                      </button>

                      <div className="my-1 border-t border-slate-100 dark:border-white/5" />

                      <button
                        onClick={() => {
                          setActiveMenuId(null);
                          onDelete(event);
                        }}
                        className="flex w-full items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all"
                      >
                        <Trash2 size={13} />
                        Delete Event
                      </button>
                    </div>
                  )}
                </td>
              </tr>
            );
          })}
        </tbody>
      </table>
    </div>
  );
}
